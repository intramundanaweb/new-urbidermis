export default {
  ...
  methods: {
    ...
    createNew (entity, toCustomer) {
      ...
      this.entity = entity
      if (this.type === 'email') {
        this.notificationContent = toCustomer ? this.$root.labels.default_content_customer : this.$root.labels.default_content_employee
      } else {
        this.notificationContent = toCustomer ? this.$root.labels.default_content_customer_sms : this.$root.labels.default_content_employee_sms
      }
      this.notificationSubject = entity === 'event' ? '%event_name%' : '%service_name%'
