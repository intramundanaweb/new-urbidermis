<template>
  <div class="am-show-calendar-container">
    <div
      ...
      :class="formObjData ? `am-form-${formType}-${$options.name}-${addToCalendarData.bookableType}` : ''"
    >
      <!-- Congratulations -->
            ...
            <g stroke="none" stroke-width="1" :fill="addToCalendarData.color" fill-rule="evenodd">
              <g id="trophy-2" :fill="addToCalendarData.color">
                ...
                <path
                  ...
                  id="Shape"
                  :fill="formObjData ? formsData.globalSettings.formBackgroundColor : '#ffffff'"
                ></path>
                <path
                  id="Shape"
                  :fill="addToCalendarData.color"
            ...
            <g stroke="none" stroke-width="1" :fill="trophyColor()" fill-rule="evenodd">
              <g id="trophy-2" :fill="trophyColor()">
                ...
                <path
                  d="M48.8443002,15.7224529 C45.5005423,6.20144843 46.3007112,6.42871364 43.03539,15.7224529 C32.8222466,15.9270203 33.3325159,15.2730941 41.2359437,21.2032508 C38.2829325,30.8598674 37.785305,30.199333 45.9401324,24.5849342 C54.312169,30.3481616 53.5240673,30.6147888 50.6431719,21.2032508 C58.7816224,15.0978327 58.7603612,15.9209867 48.8443002,15.7224529 L48.8443002,15.7224529 Z"
                  id="Shape"
                  :fill="formObjData ? formObjData.globalSettings.formBackgroundColor : '#ffffff'"
                ></path>
                <path
                  id="Shape"
                  :fill="trophyColor()"
      ...
      >
        <el-col
          class="el-form-item"
          :class="formObjData ? `am-select-${formType}-${$options.name}-${addToCalendarData.bookableType}`: ''"
          :sm="12"
        >
          <el-select
            v-model="calendarIndex"
            :popper-class="formObjData ? `am-dropdown-${formType}-${$options.name}-${addToCalendarData.bookableType}` : ''"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      trophyColor () {
        if (this.formObjData && this.addToCalendarData.bookableType !== 'event') {
          return this.formObjData.globalSettings.formImageColor
        } else {
          return this.$root.settings.customization.primaryColor
        }
      },
