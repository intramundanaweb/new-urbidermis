      ...
      <el-form v-if="mounted && appointment !== null" :model="appointment" ref="appointment" :rules="rules" label-position="top">
        <el-tabs v-model="newAppointmentTabs" @tab-click="handleTabClick">
          ...
          <el-tab-pane :label="$root.labels.schedule" name="schedule">
            ...
            <!-- Service Coupon -->
            <el-form-item :label="$root.labels.apply_coupon" :class="{active:categorySpinnerActive}"
                          v-if="couponsFilteredService.length>0 && showCoupon && ($root.settings.role === 'admin' || $root.settings.role === 'manager')">
              <el-select
                  v-model="coupon"
                  filterable
                  clearable
